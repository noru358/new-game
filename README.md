...
The current temple region is 3840 × 2160 with four named spaces, thirteen columns, six water segments, seven low walls, and eight enemies. It uses the chosen 2:1 diamond ground and prop tops, camera zoom 1.25, and ground-position Y-sort. Large built structures follow the diamond axes; natural boundaries may vary. The minimap shows the region, visible camera area, player, and living enemies. These are 2D projection sketches and a blockout, not final art or a final map size. The earlier 2400 × 1400 practice field remains in `game/view_prototype.tscn`.

The starting waterside court is the first structured area: a central combat court, two openings through the canals, a gate between pylons, and a stair to a raised temple terrace. Dark water and terrace side walls block movement; lighter irregular wet banks are walkable. The minimap uses one neutral ground color because its previous three vertical color bands were arbitrary map-coordinate ranges. The other three named areas remain rough blockouts pending review of this first court.

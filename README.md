...
The separate **Hybrid Court v07** comparison scene uses the existing 2D combat and navigation simulation with a 3D orthographic presentation. Run `godot --path . res://game/hybrid_height.tscn`, or export with `macOS Height Lab` / `Windows Height Lab`. The normal launch scene remains the previous temple prototype. This first-court slice has three elevations, four ramps, six connected stepping stones, two deep-water cuts flanking the southern approaches with dry detours, a height-and-route minimap, and the accepted eight-enemy, five-role mix. It starts with the baseline two-hit combo and two dash charges. Enemy defeats grant XP immediately; level-ups pause for a three-card choice, with `U_CHAIN` unlocking hits 3 and 4. Multiple wisps, orbit contact, chain arcs and the automatic seal have 3D presentation. Wisps follow a balanced formation relative to player facing. The third hit visibly pulls on connected slopes and stops at cliff walls. The lab uses its own local unlock-progress save so it does not change the main game's cumulative card unlocks. Landmark buttons jump to each approach and the stepping stones; Tab toggles an overview, N respawns enemies while retaining XP/cards, and R resets the scene. Esc resumes after focus loss. The omitted systems and proposed order are tracked in `HYBRID_REINTEGRATION_CHECKLIST.md`.

`HybridTerrain` is the common source for ground heights, the six stepping stones, deep-water footprints, 3D surfaces, 2D barriers and the minimap. Gameplay coordinates represent the ground plane; render positions add sampled elevation. Cliff and water barriers block movement, melee and wisp shots, while connected paths permit combat. The airborne attack guide shows effective reach including enemy collision radius and stops at a blocking cliff. Terrain depth hides the part of a slash behind a wall. Actor positions, camera follow and slash progress are interpolated for each rendered frame, while hit detection remains in the 2D physics tick. Enemy charge and shot warnings stay in level air planes, while blocked portions of area rings are omitted without compressing their radius. Depth testing hides effects behind terrain. Projectile visuals interpolate smoothly between launch and target elevations. Sprite feet and ground shadows follow the sampled surface; the larger hybrid collision clearance keeps bodies outside cliff faces, and the wisp stays on the player's side. Screen-space feel and art remain subject to play review. Overlapping walkable floors are outside this prototype.
...
godot --headless --path . --script res://tests/verify_hybrid_growth.gd
godot --headless --path . --script res://tests/verify_hybrid_combat_feedback.gd
```

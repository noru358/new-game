...
func setup(step: int, hurt: bool, direction: Vector2) -> void:
	...
	duration = 0.22 if hurt or step == 4 else 0.16
	z_index = 10
...
func _draw() -> void:
	...
	var tone := Color("ff9e91") if is_hurt else Color("fff0b4") if combo_step == 4 else Color("e5aaff") if combo_step == 3 else Color("bdfaff")
	var ring := tone
	ring.a = (1.0 - progress) * (0.7 if is_hurt else 0.8)
	draw_arc(Vector2.ZERO, 7.0 + progress * (30.0 if is_hurt or combo_step == 4 else 22.0), 0.0, TAU, 24, ring, 3.0)
	var core := tone
	...
	for i in range(5):
		...
		var outer := ray * (17.0 + progress * (27.0 if combo_step == 4 else 18.0))
		draw_line(inner, outer, ring, 2.5 if combo_step == 4 else 2.0)

...
func setup(step: int, hurt: bool, direction: Vector2) -> void:
	...
	duration = 0.22 if hurt or step == 3 else 0.16
	z_index = 10
...
func _draw() -> void:
	...
	draw_arc(Vector2.ZERO, 7.0 + progress * (30.0 if is_hurt or combo_step == 3 else 22.0), 0.0, TAU, 24, ring, 3.0)
	var core := tone
	...
	for i in range(5):
		...
		var outer := ray * (17.0 + progress * (27.0 if combo_step == 3 else 18.0))

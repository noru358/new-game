...
func _run() -> void:
	...
	var clear_bridge: bool = scene.arena_navigation.has_clear_path(Vector2(900, 700), Vector2(900, 970))
	var blocked_water: bool = not scene.arena_navigation.has_clear_path(Vector2(600, 570), Vector2(600, 850))
	_check(clear_bridge and blocked_water, "water channel has a broad usable opening")
	var ray := PhysicsRayQueryParameters2D.create(Vector2(600, 570), Vector2(600, 850), 4)
	...
	_check(not scene.arena_navigation.find_path(Vector2(600, 500), Vector2(600, 900)).is_empty(), "north bank connects through the crossing")
	...
	player.global_position = Vector2(600, 890)
	player.hurt_immunity = 100.0
	chaser.global_position = Vector2(600, 520)

...
func _run() -> void:
	...
	_check(scene._constrain_wisp(target.position).is_equal_approx(scene.player.position), "companion cannot drift across cliff and shoot from the other level")
	...
	scene.queue_free()
	await _frames(3)
	if failures == 0: print("Hybrid height verification passed: four ramps, terrace/walking/dash barriers, melee, wisp targeting/projectiles, pursuit, visual height and pause")

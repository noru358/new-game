...
func _run() -> void:
	...
	_check(scene.terrain.surface_name(Vector2(1290, 1640)) == "바위길" and is_equal_approx(scene.terrain.height_at(Vector2(1290, 1640)), 80.0), "stone path shares the same single-surface height data")
	scene.teleport(Vector2(1290, 1840))
	await _walk(Vector2.UP, 90, scene.player)
	_check(scene.terrain.height_at(scene.player.position) == 160.0, "stone path provides a second walkable connection to the courtyard")
	scene.teleport(Vector2(1290, 1640))
	await _walk(Vector2.RIGHT, 30, scene.player, true)
	_check(scene.player.position.x < 1370.0, "stone path side stops a dash into the cliff")
	...
	scene.queue_free()
	await _frames(3)
	if failures == 0: print("Hybrid height verification passed: four ramps, stone path, terrace/walking/dash barriers, melee, wisp targeting/projectiles, pursuit, visual height and pause")
